...
export default {
  name: 'engineeringHouseKeeperManage',
  data () {
    return {
        listRes: '', //服务器端查询的数据
        //需要查询的字段
  ...
  methods: {
    edit: function(id, event){
      ...
      //给目前的实例注册一个事件
      //alert(1);
      var obj = {id: id, viewName: 'ehkmEdit'};
    ...
    search: function(){
      ...
      var urlbase = this.$http.options.root;
      //请求的URL
      var resUrl = urlbase+'/user/api/projectManagerProfiles?page=0&size='+this.perSize+'&sort=userId,ASC&filter='+this.filterString;
    ...
    ccp: function(value){
      ...
      if(this.filterString){
        var resUrl = urlbase+'/user/api/projectManagerProfiles?page='+page+'&size='+this.perSize+'&sort=userId,ASC&filter='+this.filterString;
      }else{
        var resUrl = urlbase+'/user/api/projectManagerProfiles?page='+page+'&size='+this.perSize+'&sort=userId,ASC';
      }
